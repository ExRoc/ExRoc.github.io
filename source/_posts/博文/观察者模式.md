...
banner_img: /img/cover/观察者模式.png
author: ExRoc
categories: ["博文"]
tags: ["设计模式", "字节跳动", "技术分享"]
...
上一段代码已经实现了观察者模式，但其中存在一个 bug，如果▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇ （bug 描述被 ▇▇ 隐藏，建议思考之后，再点击下方折叠内容查看），为了解决这个 bug，下面提供了两种方案（当然最简单的方案是令布告板类中的 `observerState` 类型为指针，初始值为 `NULL` 表示未更新过，不过为了介绍关于观察者模式的更多实现，请读者耐心往下看~）。

{% fold info @▇▇隐藏内容 %}

如果首次更新的 `temperature` 或 `humidity` 值为 0，那么 `ccBoard` 与 `sBoard` 将不会刷新
...
```bash
git clone git@github.com:ExRoc/observer_mode_sample.git
```
